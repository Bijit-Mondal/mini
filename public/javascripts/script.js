...
function ShowResults(response){
    ...
    '<div class="flex one">'+
      '<div>'+
        ...
        '</a>'+
            '<footer>'+
              '<h3>TITLE</h3>'+
                '<button class="warning" onclick="DownloadAudio(`VIDEO_ID`,`TITLE`)">Download Audio</button>'+
                '<button class="success" onclick="DownloadVideo(`VIDEO_ID`,`TITLE`)">Download Video</button>'+
                    '</footer>'+
                    '</article>'+
                  '</div>'+
                '</div>'+
  ...
  let thumbnail_url = 'https://i.ytimg.com/vi/g1ARRcK4LVs/default.jpg?sqp=-oaymwEjCPYBEIoBSFryq4qpAxUIARUAAAAAGAElAADIQj0AgKJDeAE=&rs=AOn4CLBWlofY3X_-ZRbw7i4ga1EddwSKpA';
  ...
  for(let i=0;i<json.items.length;i++){
      ...
      thumbnail_url = json.items[i].snippet.thumbnails.medium.url;
